...
class App extends Component {
  ...
  async componentWillMount() {
    ...
    // let _amount = '100000000000000000'
    // let _spender = this.state.userAccount
    // this.state.token.methods.approve(this.state.ethSwap.address, _amount).send({ from: _spender }).on('transactionHash', (hash) => {
    //   setTimeout(() => {
    //     let _amount = '100000000000000000'
    //     const receipt = this.state.tokenFarm.methods.stakeTokens(_amount).send({ from: _spender })
    //     console.log('111111111111111112222222222222222222', receipt);
    //     console.log('111111111111111112222222222222222222', receipt.events);
    //   }, 10000);
    // })
  ...
  stakeTokens = async (tokenAmount) => {
    if (this.state.SelectStakePeriod === null) {
      ...
      if (window.web3.utils.fromWei(tokenAmount, 'Ether') > Number(window.web3.utils.fromWei(this.state.tokenBalance, 'Ether'))) {
        ...
        if (this.state.SelectStakePeriod === '06 Months') {
          ...
          timeWhichAdd = 732
        } else if (this.state.SelectStakePeriod === '2 Mnts') {
          timeWhichAdd = 0
        }
        ...
        await this.state.token.methods.approve(this.state.ethSwap.address, tokenAmount).send({ from: this.state.userAccount }).on('transactionHash', (hash) => {
          ...
          setTimeout(() => {
            this.setState({ loading: true })
            this.state.ethSwap.methods.stakeTokens(tokenAmount, 1).send({ from: this.state.userAccount }).on('transactionHash', (hash) => {
              ...
              if (e.code === 4001) {
                window.location.reload()
                console.log('errorrrrrrrrrr1111')
              }
            })
            this.setState({ loading: true })
        ...
        //   if (this.state.transactionRejected !== true) {
  ...
  render() {
    ...
    if (this.state.SelectStakePeriod != '' && this.state.SelectStakePeriod != null) {
      if (this.state.SelectStakePeriod === '12 Months') {
        ...
        SelectStakePeriodContent = <span>12 <span>Months</span></span>
      } else if (this.state.SelectStakePeriod === '2 Mnts') {
        SelectStakePeriodContent = <span>2 min</span>
      } else {
    ...
    if (this.state.receiptDiv1 === true && this.state.receiptDiv2 === false) {
      receiptDiv1 = <div className="wholePage">
        <br /><br />
    ...
    if (this.state.receiptDiv2 === true && this.state.receiptDiv1 === false) {
      receiptDiv2 = <div className="wholePage">
        <br /><br />
        <div className="receiptMain">
    ...
    if (this.state.loading === false && this.state.userAccount !== '') {
      StakeBtn = <button className='stakeBtnss stakePageStakeBTN' onClick={(event) => {
        console.log('Invalid Stake Amount Condition: ', this.input.value, window.web3.utils.fromWei(this.state.tokenBalance, 'Ether'));
        // if (this.input.value === '' || this.input.value === null || this.input.value === undefined || this.input.value === '0' || this.input.value === 0 || this.input.value >= window.web3.utils.fromWei(this.state.tokenBalance, 'Ether')) {
      ...
      } style={{ backgroundColor: '#2223DE', border: '2px solid #023433', borderRadius: '11px', width: '170px', height: '40px', fontSize: '17px', marginTop: '3px', marginBottom: '1px' }}>Stake</button>
    } else if (this.state.userAccount === '') {
      StakeBtn =
        <Link to={{ pathname: '/WhichWallet' }}>
          <button className='stakeBtnss stakePageStakeBTN' style={{ backgroundColor: '#2223DE', border: '2px solid #023433', borderRadius: '11px', width: '170px', height: '40px', fontSize: '17px', marginTop: '3px', marginBottom: '1px' }}>Connect Wallet</button>
        </Link>
    } else {
      StakeBtn = <button className='stakeBtnss stakePageStakeBTN' style={{ backgroundColor: '#2223DE', border: '2px solid #023433', borderRadius: '11px', width: '170px', height: '40px', fontSize: '17px', marginTop: '3px', marginBottom: '1px' }}>Loading...</button>
    }
    ...
    if (this.state.loading === false && this.state.userAccount !== '') {
      StakeBtnDesktop =
        <p style={{ textAlign: 'center' }}>
          <button className='stakeBtns'
            style={{ backgroundColor: '#2223DE', marginTop: '5px', border: '3px solid #00339A' }}
            onClick={(event) => {
              let tokenamountNow = window.web3.utils.fromWei(this.state.tokenBalance, 'Ether')
              // if (this.inputA.value.toString() > tokenamountNow || this.inputA.value === null || this.inputA.value === undefined || this.inputA.value === '0' || this.inputA.value === 0 || this.inputA.value === '') {
              if (this.inputA.value === null || this.inputA.value === undefined || this.inputA.value === '0' || this.inputA.value === 0 || this.inputA.value === '') {
                alert('Invalid Stake Amount')
              } else {
                event.preventDefault()
      ...
      StakeBtnDesktop =
        <Link to={{ pathname: '/WhichWallet' }}>
          <button
            style={{ backgroundColor: '#2223DE', marginTop: '5px', border: '3px solid #00339A' }}
            className='stakeBtns'>Connect Wallet</button>
        </Link>
    } else {

      StakeBtnDesktop = <button className='amountStakeBtns' style={{ backgroundColor: '#2223DE', marginTop: '5px' }}>Loading...</button>
    ...
    let FormaterTokenBln = window.web3.utils.fromWei(this.state.tokenBalance, 'Ether')
    let stateTokenBln = Number(FormaterTokenBln).toFixed(4)
    let stakeToknBlnc
    if (stateTokenBln === '0.0000' || stateTokenBln === 0.0000) {
      stakeToknBlnc = '0'
    } else {
      stakeToknBlnc = stateTokenBln
    }
    let paramsDataContent = <h4 className='colorGreyy stakeBlance' style={{ marginBottom: '4px', color: '#39393B', fontSize: '16px' }}><span style={{ marginRight: '6px' }}>Balance: </span>{stakeToknBlnc}</h4>
    ...
    if (StakePeriodRate === '24 Months') {
      ratePerAnnum = '24 Months @ 200% Per Year'
    } else if (StakePeriodRate === '12 Months') {
      ratePerAnnum = '12 Months @ 100% Per Year'
    } else if (StakePeriodRate === '06 Months') {
      ratePerAnnum = '06 Months @ 25% Per Year'
    } else {
              ...
              <div className="col-4">
                ...
                <div className="bottomHeaderr functionalityDiv">
                  ...
                  <div className='DeskContentDiv' id='hello' style={{ paddingLeft: '20px', paddingRight: '20px' }}>
                      ...
                      <div className="row">
                        <div className="col-7">
                          ...
                          <span className='MaxTxtt' style={{ cursor: 'pointer', color: '#1767FE' }} onClick={(event) => {
                            let ftpAccountBalance = window.web3.utils.fromWei(this.state.tokenBalance, 'Ether')
                            this.inputA.value = Number(ftpAccountBalance).toFixed(5);
                            ...
                            if (this.state.SelectStakePeriod === '24 Months') {
                              setTimeout(() => {
                                let extraStakeAmountt = ((tokenAmountA / 100) * 100) * 2
                              ...
                              setTimeout(() => {
                                let extraStakeAmountt = (tokenAmountA / 100) * 100
                                console.log('7777777777777', extraStakeAmountt.toString().length);
                                let extraStakeAmount
                                if (extraStakeAmountt.toString().length > 10) {
                                  extraStakeAmount = extraStakeAmountt.toFixed(5)
                              ...
                              setTimeout(() => {
                                ...
                                console.log('extraStakeAmountextraStakeAmount', tokenAmountA);
                                this.setState({ extraStakeAmount })
                                this.setState({ outputA: parseFloat(tokenAmountA) + parseFloat(extraStakeAmount) })
                                console.log('outputA', this.state.outputA);
                              }, 1);
                        ...
                        <div className="col-5 whiteStakeAmountt">
                          <h5 style={{ color: '#0367FE', fontSize: '15px' }} className='swapAmountTxt'>Stake<span style={{ color: '#fff' }}>.</span>Amount</h5>
                    ...
                    <h4 className='colorGreyy twelveMntTxtt perAnnum' style={{ color: '#39393A', marginTop: '-22px', fontSize: '16px' }}>{ratePerAnnum}</h4>
                    <div className="darkBlueDiv" style={{ backgroundColor: 'black' }}>
                      <div className="row">
                        <div className="col-6 extraFtpTxt" style={{ position: 'relative', left: '-18px' }}>
                          ...
                          <div className="row">
                            <div className="col-4">
                              <img className='extraImg stakeExtraFtpTokenImg' src="https://fairtrader.io/wp-content/uploads/2021/08/cropped-favicon-192x192.png" width='35px' alt="logo" />
                            </div>
                            <div className="col-8">
            ...
            <div>
              <div className="bottomHeaderrr" style={{ paddingTop: '5px' }}>
                ...
                <div className='DeskContentDiv' id='helloo' style={{ paddingLeft: '20px', paddingRight: '20px' }}>
                  ...
                  <div className="whiteDivv">
                    <div className="row swapAmountBox">
                      <div className="col-7">
                        ...
                        <span className='MaxTxtt' style={{ cursor: 'pointer', color: '#1167FB' }} onClick={(event) => {
                          let ftpAccountBalance = window.web3.utils.fromWei(this.state.tokenBalance, 'Ether')
                          this.input.value = Number(ftpAccountBalance).toFixed(5);
                  ...
                  <br />
                  <h4 className='colorGreyy stakeBlance' style={{ color: '#39393B', marginTop: '-10px', marginBottom: '-8px', fontWeight: 'bold' }}>
                    unlock date: {dateOfUnlock}
                  </h4>
                  <br />
                  <p style={{ textAlign: 'center', color: '#1dccff', fontSize: '18px', marginTop: '-2px' }}>
                    {StakeBtn}
                  </p>
                </div>
              </div>
